...
while True:
    ...
    results = model(imgRegion, stream=True) #recomendado stream
    ...
    for r in results:
        ...
        for box in boxes:
            ...
            #cv2.rectangle(img, (x1, y1), (x2,y2), (255,0,255), 3) #this is for open csv
            w, h = x2-x1, y2-y1
            #cvzone.cornerRect(img, (x1,y1,w,h), l=10)

            #Confidence
            conf = math.ceil((box.conf[0] * 100))/100 #usamos math para manejar decimales
